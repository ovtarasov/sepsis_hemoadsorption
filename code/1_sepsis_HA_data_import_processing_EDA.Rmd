...
NA_heatmap <-	function(sepsis_0_status, count_NA_vector) {
  ...
  study_vlines <- c(study_vlines, 
	...
	geom_text(
		data = data.frame(study = c(levels(sepsis_NA_tibble$study_id)[-5], "1C_1"), 
	...
	geom_rect(
		xmin = -0.25, 
		xmax = max(study_vlines) + 0.25, 
		ymin = min(NA_hlines) - 0.25, 
...
sepsis_NA_heatmap <- NA_heatmap(sepsis_0_status = sepsis_0_status, count_NA_vector = count_NA_vector) +
	# Label frame
	geom_text(
	...
	# Set margin width
	theme(plot.margin = margin(25, 15, 10, 35, "pt")) +
	# Write plot title
