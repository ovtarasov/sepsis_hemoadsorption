...
sepsis_HA_df_analyze <- sepsis_HA_df_analyze %>% 
  ...
  mutate(NLR_category = factor(NLR_category, levels = c("Neutropenia", "Normal range", "Grey zone", "Mild to moderate inflammation", "Moderate to severe inflammation", "Severe inflammation", "Critical inflammation: septic shock", "Critical inflammation: politrauma, cancer, surgery"))) %>%
...
sepsis_HA_df_analyze <- sepsis_HA_df_analyze %>% 
  mutate(
    death_category = case_when(
      ICU_first_end_status == "death" & ICU_dur_T0_first_end >= 0 & ICU_dur_T0_first_end <= 6 ~ "early mortality",
      ICU_first_end_status == "death" & ICU_dur_T0_first_end > 6 ~ "late mortality",
      ICU_first_end_status == "recovery" ~ "recovery")) %>% 
  mutate(death_category = factor(death_category, levels = c("early mortality", "late mortality", "recovery"))) 
```

### 3. Histograms of variable distribution

Log transformation is required for: WBC, PLT, creatinine, VIS2020, NEU, LYM, bilirubin_total, NLR, SII, CRP, PCT, shock_dur_T0_first_end, ICU_dur_T0_first_end, MV_dur_T0_first_end, lactate, aPTT; not required for: age, APACHE2, SOFA, PF_ratio, weight, MAP, fibrinogen.
...
selected_vars <- sepsis_HA_df_analyze %>%
  filter(time_point == 0) %>%
  dplyr::select("age", "SOFA", "MAP", "WBC", "PLT", "creatinine", "VIS2020", 
                "NEU", "LYM", "bilirubin_total", "NLR", "SII", "weight", 
                "CRP", "PF_ratio", "PCT", "APACHE2", "shock_dur_T0_first_end", "ICU_dur_T0_first_end", "MV_dur_T0_first_end", "lactate", "fibrinogen", "aPTT") %>% pivot_longer(cols = everything(), names_to = "Variable", values_to = "Value")
...
sepsis_HA_df_analyze_log <- sepsis_HA_df_analyze %>%
  filter(time_point == 0) %>%
  mutate(across(c(WBC, PLT, creatinine, VIS2020, NEU, LYM, bilirubin_total, 
                  NLR, SII, CRP, PCT, shock_dur_T0_first_end, 
                  ICU_dur_T0_first_end, MV_dur_T0_first_end, lactate, aPTT),
                ~ add_epsilon(.))) 

sepsis_HA_df_analyze_log_longer <- sepsis_HA_df_analyze_log %>%
    dplyr::select("age", "SOFA", "MAP", "WBC", "PLT", "creatinine", "VIS2020", 
                "NEU", "LYM", "bilirubin_total", "NLR", "SII", "weight", 
                "CRP", "PF_ratio", "PCT", "APACHE2", "shock_dur_T0_first_end", "ICU_dur_T0_first_end", "MV_dur_T0_first_end", "lactate", "fibrinogen", "aPTT") %>% 
...
```{r}
death_count <- sum(sepsis_HA_df_analyze$ICU_first_end_status == "death" & sepsis_HA_df_analyze$time_point == 0)
total_count <- sum(sepsis_HA_df_analyze$time_point == 0)
mortality_rate <- (death_count / total_count) * 100

cat("Total patients:", total_count, "\n", "Mortality rate:", round(mortality_rate, 1), "%\n")
```
...
Statistically significant variables in the baseline comparison (unadjusted univariate logistic regression analysis): age, APACHE II, SOFA, PLT, DDimer, PF_ratio, albumin, creatinine, lactate.
...
Statistically significant predictors of death: sex, age, MV_T0, SOFA, VIS2020, APACHE2, lactate.

1.  **sex**: Male have 2.01 times higher odds of death compared to female.

2.  **age**: For every additional year of age, the odds of death increase by 4% (OR = 1.04).

3.  **MV_T0**: Being on mechanical ventilation substantially increases the risk of death (2.56 times higher odds).

4.  **SOFA**: Higher SOFA scores are strongly associated with increased mortality risk. Each one-point increase in the SOFA score is associated with a 21% increase in the odds of death (OR = 1.21).

5.  **VIS2020**: Even slight increases in VIS2020 are linked to higher odds of death. Although the effect size is small, this result is statistically significant.

6.  **APACHE2**: Higher APACHE2 scores are significant predictors of increased mortality. Each one-point increase in the APACHE2 score increases the odds of death by 17% (OR = 1.17).

7.  **lactate**: Elevated lactate levels are strongly associated with an increased risk of death in patients with sepsis. Each one-unit increase in lactate is linked to a 40% increase in the odds of death (OR = 1.40).
...
predictors <- c("sex", "age", "shock_T0", "MV_T0", "SOFA", "MAP", "WBC", "PLT", "creatinine", "VIS2020", "NEU", "LYM", "bilirubin_total", "NLR", "SII", "weight", "CRP", "PF_ratio", "PCT", "APACHE2", "shock_dur_T0_first_end", "ICU_dur_T0_first_end", "MV_dur_T0_first_end", "lactate", "fibrinogen", "aPTT")
...
all_results <- lapply(time_points, function(tp) {
  ...
  results <- lapply(predictors, function(var) {
    formula <- as.formula(paste("ICU_first_end_status ~", var, "+ combined_efferon"))
...
label_text <- cbind(
  ...
  CI = sprintf("[%.2f, %.2f]", forest_data_time_point_0$conf.low, forest_data_time_point_0$conf.high),
...
Statistically significant predictors of death: sex, age, MV_T0, SOFA, ***+creatinine***, VIS2020, APACHE2, ***+shock_dur_T0_first_end, +MV_dur_T0_first_end***, lactate.
...
5.  **creatinine**:Higher **log-transformed** creatinine levels are significantly associated with increased mortality risk. Each one-unit increase in the log-transformed creatinine value corresponds to 1.74 times higher odds of death (OR = 1.74).

6.  **VIS2020**: **Log-transformed** VIS2020 values are significantly associated with mortality risk. A one-unit increase in the log-transformed VIS2020 is linked to a 17% increase in the odds of death (OR = 1.17).

7.  **APACHE2**: Higher APACHE2 scores are significant predictors of increased mortality. Each one-point increase in the APACHE2 score increases the odds of death by 17% (OR = 1.17).

8.  **shock_dur_T0_first_end**: **The log-transformed** duration of shock is significantly associated with increased mortality risk. Each one-unit increase in the log-transformed shock duration results in 1.42 times higher odds of death (OR = 1.42).

9.  **MV_dur_T0_first_end**: **Log-transformed** duration of mechanical ventilation is significantly associated with increased mortality risk. A one-unit increase in the log-transformed mechanical ventilation duration corresponds to 1.40 times higher odds of death (OR = 1.40).

10. **lactate**: **Log-transformed** lactate levels show the strongest association with mortality risk. A one-unit increase in the log-transformed lactate is associated with 3.52 times higher odds of death (OR = 3.52).
...
label_text_log <- cbind(
  ...
  CI = sprintf("[%.2f, %.2f]", results_log_df$conf.low, results_log_df$conf.high),
...
Elevated lactate levels statistically significant increase the odds of death. Variables such as combined_efferon, sex, age, MV_T0, SOFA, VIS2020, and APACHE2 did not show a statistically significant association with mortality in this model.

```{r, fig.width=8, fig.height=8}
sepsis_HA_df_analyze_time_point_0 <- sepsis_HA_df_analyze %>%
  filter(time_point == 0)
  
multivariate_model <- glm(ICU_first_end_status ~ combined_efferon + sex + age + MV_T0 + SOFA + VIS2020 + APACHE2 + lactate, data = sepsis_HA_df_analyze_time_point_0, family = binomial)
