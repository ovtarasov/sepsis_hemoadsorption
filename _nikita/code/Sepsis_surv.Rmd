...
ft_num <- sepsis_clear %>%
  ...
  summarise(across(everything(), list(
    ...
    "Максимум" = ~ as.character(ifelse(all(is.na(.)), NA, round(max(., na.rm = TRUE), 2)))), .names = "{.col}::{.fn}")) %>%
...
summary_stats_table <- sepsis_clear %>%
  ...
  separate(Variable, into = c("Variable", "Statistic"), sep = "::", fill = "right") %>%
  pivot_wider(names_from = Variable, values_from = Value, values_fn = list) %>%
  rename ("Терапия" = treatment, "Статистика" = Statistic) %>%
  mutate(across(where(is.list), ~ sapply(., function(x) if (length(x) > 1) paste(x, collapse = "; ") else x))) %>%
  flextable() %>%
  theme_vanilla() %>%
  autofit() %>%
...
summary_stats_cat_table <- sepsis_clear %>%
  ...
  pivot_longer(cols = -treatment, names_to = "Variable", values_to = "Category") %>%
  filter(!is.na(Category)) %>%
  ...
  ungroup() %>%
  rename("Переменная" = Variable, "Категория" = Category, "Терапия" = treatment) %>%
...
summary_stats_table <- sepsis_clear %>%
  ...
  pivot_longer(cols = -c(Терапия, Исследование), names_to = "Переменная", values_to = "Значение") %>%
  separate(Переменная, into = c("Переменная", "Статистика"), sep = "::", fill = "right") %>%
  pivot_wider(names_from = Переменная, values_from = Значение, values_fn = list) %>%
  mutate(across(where(is.list), ~ sapply(., function(x) paste(unique(x), collapse = "; ")))) %>%
  flextable() %>%
  theme_vanilla() %>%
  autofit() %>%
  hline_top(border = officer::fp_border(color = "black", width = 1)) %>%
...
summary_stats_cat_table <- sepsis_clear %>%
  ...
  pivot_longer(cols = -c(treatment, study_id), names_to = "Variable", values_to = "Category") %>%
  filter(!is.na(Category)) %>%
  ...
  ungroup() %>%
  rename("Переменная" = Variable, "Категория" = Category, "Терапия" = treatment, "Исследование" = study_id) %>%
  select(Терапия, Исследование, Переменная, Категория, Количество, Общее, Доля) %>%
  flextable() %>%
  theme_vanilla() %>%
  autofit() %>%
  border_remove() %>%
